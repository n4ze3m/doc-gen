...
export const POST = async (request: NextRequest) => {
    ...
    const apiKey = request.headers.get("x-api-key") as string | null;
    ...
    const userExist = await convexServer.query(api.apikey.getApiKey, {
        apiKey: apiKey,
    });
    ...
    if (docInfo.user_id !== userExist.userId) {
        return NextResponse.json(
            {
                error: "Invalid api key",
            },
